#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Split a generated 2x2 walk-sheet into review-only transparent PNG frames.

This tool is deliberately narrow: it handles a newly generated sheet that has
no watermark or text, removes its declared chroma key, and keeps the output
inside ``assets/candidates``.  It never writes to the formal runtime manifest.
"""

from __future__ import annotations

import argparse
import json
from pathlib import Path

from PIL import Image


PROJECT_ROOT = Path(__file__).resolve().parents[1]
KEY = (0, 255, 0)


def remove_green(image: Image.Image) -> Image.Image:
    """Convert a pure chroma-key background to alpha without touching art."""

    rgba = image.convert("RGBA")
    pixels = rgba.load()
    for y in range(rgba.height):
        for x in range(rgba.width):
            red, green, blue, alpha = pixels[x, y]
            distance = max(abs(red - KEY[0]), abs(green - KEY[1]), abs(blue - KEY[2]))
            if distance <= 18 or (green > red + 36 and green > blue + 36):
                pixels[x, y] = (0, 0, 0, 0)
    return rgba


def main() -> None:
    parser = argparse.ArgumentParser(description="切分 2x2 走路候选并去除绿色键控背景。")
    parser.add_argument("--input", type=Path, required=True)
    parser.add_argument("--candidate", choices=("candidate_a", "candidate_b"), required=True)
    parser.add_argument("--facing", choices=("left", "right"), required=True)
    args = parser.parse_args()

    source = args.input.resolve()
    if not source.is_file():
        parser.error(f"找不到候选图：{source}")
    destination = PROJECT_ROOT / "assets" / "candidates" / "walk_side" / args.candidate
    expected = [destination / f"walk_side_{index:02d}.png" for index in range(4)]
    if any(path.exists() for path in expected):
        parser.error("候选帧已存在；为避免覆盖，请使用新的候选名称。")

    with Image.open(source) as raw:
        sheet = raw.convert("RGBA")
    if sheet.width % 2 or sheet.height % 2:
        parser.error("候选图必须是严格的 2x2 网格（宽和高都可被 2 整除）。")

    cell_width, cell_height = sheet.width // 2, sheet.height // 2
    destination.mkdir(parents=True, exist_ok=True)
    for index in range(4):
        col, row = index % 2, index // 2
        frame = sheet.crop(
            (col * cell_width, row * cell_height, (col + 1) * cell_width, (row + 1) * cell_height)
        )
        remove_green(frame).save(expected[index])

    metadata = {
        "candidate": args.candidate,
        "source_sheet": str(source.relative_to(PROJECT_ROOT)),
        "grid": "2x2",
        "facing": args.facing,
        "frames": [path.name for path in expected],
        "review_only": True,
        "formal_runtime": False,
        "generated_at_runtime": False,
        "notes": [
            "Generated candidate only; it must be visually reviewed before any promotion.",
            "The sheet was generated on #00FF00 and only that background was keyed to alpha.",
            "No source asset was replaced, renamed, cropped, or reconstructed.",
        ],
    }
    (destination / "candidate.json").write_text(
        json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    print(f"已写入 {destination}")


if __name__ == "__main__":
    main()
